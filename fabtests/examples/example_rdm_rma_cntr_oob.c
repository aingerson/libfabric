...
struct fid_cntr *write_cntr = NULL;
struct fid_cntr *remote_write_cntr = NULL;
struct fid_mr *mr = NULL;
void *desc;
uint64_t remote_addr, key;
char buf[BUF_SIZE];
...
static int exchange_info(void)
{
	...
	ret = sock_send(oob_sock, &key, sizeof(key));
	if (ret) {
		printf("sock_send error %d\n", ret);
		return ret;
	}

	ret = sock_recv(oob_sock, &key, sizeof(key));
	if (ret) {
		printf("sock_recv error %d\n", ret);
		return ret;
	}
	...
	if (info->domain_attr->mr_mode & FI_MR_VIRT_ADDR) {
		ret = sock_send(oob_sock, &remote_addr, sizeof(remote_addr));
		if (ret) {
			printf("sock_send error %d\n", ret);
			return ret;
		}

		ret = sock_recv(oob_sock, &remote_addr, sizeof(remote_addr));
		if (ret) {
			printf("sock_recv error %d\n", ret);
			return ret;
		}
...
static int initialize(void)
{
	...
	ret = fi_cntr_open(domain, &cntr_attr, &write_cntr, &write_cntr);
	if (ret) {
		printf("fi_cntr_open error (%d)\n", ret);
		return ret;
	}
	...
	ret = fi_cntr_open(domain, &cntr_attr, &remote_write_cntr, &remote_write_cntr);
	if (ret) {
		printf("fi_cntr_open error (%d)\n", ret);
		return ret;
	}

	ret = fi_ep_bind(ep, &remote_write_cntr->fid, FI_REMOTE_WRITE);
	if (ret) {
		printf("fi_ep_bind cq error (%d)\n", ret);
		return ret;
	}
	...
	desc = fi_mr_desc(mr);
	key = fi_mr_key(mr);
	remote_addr = info->domain_attr->mr_mode & FI_MR_VIRT_ADDR ?
			(uintptr_t) buf : 0;
	...
	if (info->domain_attr->mr_mode & FI_MR_ENDPOINT) {
		...
		ret = fi_mr_bind(mr, &remote_write_cntr->fid, FI_REMOTE_WRITE);
		if (ret)
			return ret;
...
static void cleanup(void)
{
	...
	if (write_cntr) {
		...
		if (ret)
			printf("warning: error closing write counter (%d)\n", ret);
	}

	if (remote_write_cntr) {
		ret = fi_close(&remote_write_cntr->fid);
		if (ret)
			printf("warning: error closing remote write counter (%d)\n", ret);
	}
...
static int do_write(void)
{
	...
	do {
		ret = fi_write(ep, buf, strlen(msg), desc, fi_addr, remote_addr,
			       key, NULL);
		if (ret && ret != -FI_EAGAIN) {
...
static int wait_cntr(struct fid_cntr *cntr)
{
	uint64_t val;
	int ret;
...
static int run(void)
{
	...
	if (dst_addr) {
		...
		ret = wait_cntr(remote_write_cntr);
		if (ret)
			return ret;
