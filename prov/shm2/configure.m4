...
AC_DEFUN([FI_SHM2_CONFIGURE],[
	...
	AS_IF([test x"$enable_shm" != x"no"],
	      ...
	      AS_IF([test $dsa_happy -eq 1 && test "x$with_dsa" != "xyes"],
	            [shm2_CPPFLAGS="$shm2_CPPFLAGS $dsa_CPPFLAGS $numa_LIBS"
		     shm2_LDFLAGS="$shm2_LDFLAGS $dsa_LDFLAGS $numa_LIBS"])
	      shm_LIBS="$shm2_LIBS $dsa_LIBS $numa_LIBS"

	      AC_DEFINE_UNQUOTED([SHM2_HAVE_DSA],[$dsa_happy],
				 [Whether DSA support is available])

	      AC_SUBST(shm2_CPPFLAGS)
	      AC_SUBST(shm2_LDFLAGS)
	      AC_SUBST(shm2_LIBS)
	      ])
